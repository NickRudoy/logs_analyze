...
- Отдельно анализирует трафик из Google по `Referer: google.*`.
...
```bash
./venv/bin/python3 main.py logs/ --mode error
```

Режим отдельно анализирует `*error.log*` и `*php_errors.log*`, включая `.gz`.
Он ищет признаки сканирования, DDoS/аномальной нагрузки, повторения 504, проблем
с диском/PHP-FPM и PHP-ошибок после установки капчи. Результаты сохраняются в
`results/<domain>/` как:

- `<domain>_error_report_<timestamp>.xlsx`
- `<domain>_error_summary_<timestamp>.txt`
...
- `Сводка`
- метрики Google-трафика: запросы с Google referer, доля, сессии, bounce rate
...
- `Обзор по датам` — дневной срез:
  ...
  - отказов
  - процент отказов
- `Google по датам`, `Google URL`, `Google referers`, `Google IP`, `Google детали` — отдельный срез трафика, где referer указывает на Google
...
- `--mode access|error` — обычный access-анализ или отдельный анализ error-логов
- `--error-analysis` — короткий alias для `--mode error`
